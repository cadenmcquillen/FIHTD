...
### 2.3 Gender association & pathways 
...
#### Visualization 
```{r}
top.metabs <- c(top.male.met, top.female.met)
sig_metabs_male <- male_metabolites_norm_dat[, top.metabs ]
sig_metabs_female <- female_metabolites_norm_dat[, top.metabs ]
#create boxplot, dont plot outliers
colors = c(rep("red",5),rep("blue",5))
norm_samples_boxplot <-boxplot(sig_metabs_male, main = "Top 10 Sig Metabolites in Male Samples", ylab = "log(concentration)" , outline = FALSE, show.names=TRUE, las=2)
norm_samples_boxplot <-boxplot(sig_metabs_female, main = "Top 10 Sig Metabolites in Female Samples", ylab = "log(concentration)" , outline = FALSE, show.names=TRUE, las=2)
```
...
#boneferroni
num_test <- (((335*335)-335)/2)
dat_pcor_adj$p.adj <- (dat_pcor_adj$p.value)*num_test

#label significant and insignificant values
dat_pcor_adj$sig <- vector(length = length(dat_pcor_adj$p.adj))
dat_pcor_adj$sig[dat_pcor_adj$p.adj > alpha] <- 'Not Significant'
dat_pcor_adj$sig[dat_pcor_adj$p.adj < alpha] <- 'Significant'
...
dat_pcor_adj$estimate[dat_pcor_adj$p.adj > 0.05] <- 0
dat_pcor_adj$estimate[diag(dat_pcor_adj$estimate)] <- 0
